...
** TODO PROFILE!
** TODO maybe cache is optional
** TODO Cache img in laic--list-temp-files, not just file name
** TODO Retrieve img from cache, not filename, should be faster, avoids reloading files for different overlays
* TODO [#A] Delete all tmp files on file close/emacs exit even if overlays are shown
...
* TODO [#C] Single command/keybinding C-c C-x C-l "DWIM" like org-latex-preview
